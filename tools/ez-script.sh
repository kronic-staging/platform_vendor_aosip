...
# Adapted for the Android Open Illusion Project (AOSiP) from a portion of the EZ AOSP tool
#
# AOSiP GitHub: https://github.com/AOSiP
# AOSiP Website: http://aosip.xyz
...
# Do we want to sync the latest source code from our repositories?
  if(whiptail --title "EZ Script" --yesno "Would you like to sync the latest source code from the $ROM repositories?" 10 70) then
    ...
    sleep 3
    . vendor/tools/sync.sh | tee logs/repo-sync/$ROM-$NOW.log
    sleep 3
    clear
  else
...
# Start the build while sending a log to 'logs/build'
  make kronic 2>&1 | tee logs/build/aosip_$DEVICE-$NOW.log
